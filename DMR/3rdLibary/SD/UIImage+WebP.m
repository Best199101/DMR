...
+ (UIImage *)imageWithWebPData:(NSData *)imgData error:(NSError **)error
{
    ...
    @try
    {
        decodeStatus = WebPDecode([imgData bytes], [imgData length], &config);
    }
...
+ (NSData *)imageToWebP:(UIImage *)image quality:(CGFloat)quality preset:(WebPPreset)preset
{
    
    NSParameterAssert(image != nil);
    NSParameterAssert(quality >= 0.0f && quality <= 100.0f);
    return [self convertToWebP:image quality:quality alpha:1.0f preset:preset configBlock:nil error:nil];
