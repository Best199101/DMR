...
- (void)viewDidLoad
{
    ...
    _countLabel.backgroundColor = [UIColor colorWithRed:1 green:1 blue:1 alpha:0.6];
    _countLabel.textColor = [ColorTools titleColor];
    _countLabel.textAlignment = NSTextAlignmentRight;
    _countLabel.font = [UIFont systemFontOfSize:16];
    [self.view addSubview:_countLabel];
//    _countLabel.frame = CGRectMake(self.view.frame.size.width - 200,
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    _countLabel.text = [NSString stringWithFormat:@"(%ld/%ld)   ",_imagePickerController.selectedAssetURLs.count,
...
- (BOOL)isMaximumSelectionLimitReached
{
    ...
    if (self.imagePickerController.maximumNumberOfSelection == 0) {
        return YES;
    }
...
{
    if (_imagePickerController.selectedAssetURLs.count > 0)
    {
        [_doneButton setTitleTextAttributes:
  @{NSFontAttributeName:[UIFont fontWithName:@"STHeitiSC-Light"
                                        size:18],
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else
    {
        if ([imagePickerController.delegate
             respondsToSelector:@selector(qb_imagePickerController:didSelectAsset:)])
        {
