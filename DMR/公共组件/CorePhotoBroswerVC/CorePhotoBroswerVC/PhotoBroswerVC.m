...
@property (nonatomic, strong) void(^completeCallBack)(BOOL isFinsh);
...
+(void)show:(UIViewController *)handleVC type:(PhotoBroswerVCType)type index:(NSUInteger)index photoModelBlock:(NSArray *(^)())photoModelBlock CompleteCallBack:(void (^)(BOOL))complete
{
    ...
    pbVC.completeCallBack = complete;
...
-(void)showWithPage:(NSUInteger)page{
    ...
    if (page < self.photoModels.count) {
...
- (IBAction)rightBtnClick:(id)sender {
    ...
    if(!itemView.hasImage){
        [CoreSVP showSVPWithType:CoreSVPTypeError Msg:NSLocalizedString(@"无图片数据", nil) duration:1.0f allowEdit:NO beginBlock:nil completeBlock:nil];
        return;
    }
...
//        
//        [CoreSVP showSVPWithType:CoreSVPTypeInfo Msg:NSLocalizedString(@"图片已存", nil) duration:1.0f allowEdit:NO beginBlock:nil completeBlock:nil];
//    }else{
//        
//        //展示提示框
//    }
    [CoreSVP showSVPWithType:CoreSVPTypeLoadingInterface Msg:NSLocalizedString(@"保存中", nil) duration:0 allowEdit:NO beginBlock:nil completeBlock:nil];
    
    [itemView save:^{
        
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(.5f * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            
            [CoreSVP showSVPWithType:CoreSVPTypeSuccess Msg:NSLocalizedString(@"保存成功", nil) duration:1.0f allowEdit:NO beginBlock:nil completeBlock:nil];
        ...
        [CoreSVP showSVPWithType:CoreSVPTypeError Msg:NSLocalizedString(@"保存失败", nil) duration:1.0f allowEdit:NO beginBlock:nil completeBlock:nil];
    }];
}
...
-(void)zoomOutHandle{
    ...
    //调用willDisappearView，这时候navigationbar 会设置成 hidden = NO
    
    [self.view removeFromSuperview];
    
    void(^completeCallBack)(BOOL isFinsh) = self.completeCallBack;
    if (completeCallBack) {
