...
- (void)badgeInit
{
    ...
    {
        view = [self valueForKey:@"_view"];
    }
    else
    {
        view = self.customView;
    }
    self.badgeBGColor   = [ColorTools themePinkColor];
    self.badgeTextColor = [UIColor whiteColor];
    self.badgeFont      = [UIFont systemFontOfSize:10];
    self.badgePadding   = 6;
    self.badgeMinSize   = 15;
    self.badgeOriginX   = view.frame.size.width - 7.5;
    self.badgeOriginY   = -7.5;
...
//    self.badge.frame = CGRectMake(self.badgeOriginX, self.badgeOriginY, minWidth + padding, minHeight + padding);
    self.badge.frame = CGRectMake(self.badgeOriginX, self.badgeOriginY, 15, 15);
...
-(void) setBadgeValue:(NSString *)badgeValue
{
    ...
    if (self.shouldHideBadgeAtZero == YES)
    {
