...
- (void)viewDidLoad {
    ...
    NSArray* emotionManagers = [self emotionFormessageViewController:self];
...
{
    UIBarButtonItem *leftBtn = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"btn_top_back"] style:UIBarButtonItemStylePlain target:self action:@selector(backAction)];
    self.navigationItem.leftBarButtonItem = leftBtn;
    
    UIButton *backBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    backBtn.titleLabel.font = [UIFont systemFontOfSize:12];
    [backBtn setBackgroundImage:[UIImage imageNamed:@"bg_chat_topbtn"] forState:UIControlStateNormal];
    [backBtn setTitle:@"Ta的主页" forState:UIControlStateNormal];
    [backBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [backBtn addTarget:self action:@selector(backAction) forControlEvents:UIControlEventTouchUpInside];
    [backBtn sizeToFit];
    UIBarButtonItem *exitChatBtn = [[UIBarButtonItem alloc] initWithCustomView:backBtn];
    self.navigationItem.rightBarButtonItem = exitChatBtn;
}
                           ...
                           modelForMessage:(EMMessage *)message
{
    ...
    if (message.direction == EMMessageDirectionSend) {
        model.nickname = @"";
        model.avatarURLPath = [NSString stringWithFormat:@"%@%@", KResoureURL, MainTabBar.currentUser.head_img_url];
        model.avatarImage = [UIImage imageNamed:@"avatar_shequ_default_headimg"];
    }
    else {
        model.nickname = @"";
        model.avatarURLPath = [NSString stringWithFormat:@"%@%@", KResoureURL, message.ext[@"headUrl"]];
        model.avatarImage = [UIImage imageNamed:@"avatar_shequ_default_headimg"];
    }
    return model;
}
...
- (void)backAction
{
    ...
    [[DMRChatManager sharedManager] setChatController:nil];
...
- (void)changeBarTitleWithNetStatus:(EMConnectionState)state {
    if (state == EMConnectionDisconnected) {
        self.navigationItem.title = @"网络未连接";
    }
    else {
        self.navigationItem.title = [[DMRChatManager sharedManager] getUserModelWithConversationId:self.conversation.conversationId].nickname;
    }
}
@end
